...
New here? Start with `notebooks/00_simulate_your_samples.ipynb` — it generates your own samples from scratch (photons by default; any particle via `CALOMAPS_GUN_PARTICLE`) — then read `01 → 02 → 03 → 04` in order: they build the calorimetry up from first principles and end with the resolution measured two independent ways. For the sensor-level product, `05 → 06/07` go from the full shower cascade to the validated per-crossing records (PIXELAV input decks are produced on the `pixelav-inputs` branch).
