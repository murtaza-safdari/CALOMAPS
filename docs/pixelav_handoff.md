...
(see the notebooks index) — it reads a `.npz`/deck and walks from basic sanity checks up to
the per-crossing momentum, entry-point, and angle distributions that justify every column.
